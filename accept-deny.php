...
<head>
  <meta charset="UTF-8">
  <title>Accept/Deny requests</title>
...
<body>
  ...
  <table class="responsive-table">
    <caption>Hi, <?php echo htmlspecialchars($_SESSION["name"]); ?>. Here you can accept or deny requests.</caption>
    <thead>
      <tr>
          ...
          while($fila = mysqli_fetch_row($resultado)){ //Bucle para mostrar todos los registros de una tabla
            if ($fila[6]==0) {
              ...
              <tr>
                ...
                <td id='$fila[0]' data-title='Action'><button class='$fila[0]' id='acc'>Accept</button><button class='$fila[0]' id='den'>Deny</button></td>
              </tr>
              ";
            }else if($fila[6]==1) {
              $status="Accepted";
  ...
  <script type="text/javascript">
    $("button").click(function(){
      var reqId = $(this).attr("class");
      if ($(this).attr("id")=="acc") {
        $("#"+reqId).load("accept.php",{reqId});
        location.reload();
      }
      if ($(this).attr("id")=="den") {
        $("#"+reqId).load("deny.php",{reqId});
        location.reload();
      }
    });
  </script>
